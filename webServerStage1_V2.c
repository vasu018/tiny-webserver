...
int doParse(int clientfd2, const char *readBuff2, int recvBytes) {
        ...
        if (strstr(words, "Host:") != NULL) {
            ...
            while (wordslist1 != NULL) {
                if (counter == 2) {
                    ...
                    printf("strlen of firstline %d", strlen(wordslist1));
    ...
    char stringbr[] = "<html><body> </br> </body></html>";
    sendRet1 = send(clientfd2, stringbr, strlen(stringbr), 0);
    if (firstline != NULL) {
        printf ("Host: %s\n", firstline);
    ...
    //sendRet3 = send(clientfd2, secondline, strlen(thirdline), 0);
    //if (sendRet3 < 0) {
    //    printf ("Error Sending data: %d", sendRet3);
    //}
    //else {
    //    free (thirdline); 
    //}
